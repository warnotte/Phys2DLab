 ...
 * @author Kevin Glass
 */
public class CollisionSpace implements CollisionContext, Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = -5174695173546708892L;
	...
	{
		this.collisionStrategy =new QuadSpaceStrategy(20,5);
	}
	...
	public void resolve(BodyList bodyList, float dt) {
		for (int i = 0; i < bodyList.size(); ++i)
		{
			...
			for (int j = i+1; j < bodyList.size(); ++j)
			{
				...
				if (!bi.getShape().getBounds().touches(bi.getROVPosition().getX(), 
													   bi.getROVPosition().getY(), 
													   bj.getShape().getBounds(), 
													   bj.getROVPosition().getX(), 
													   bj.getROVPosition().getY())) {
